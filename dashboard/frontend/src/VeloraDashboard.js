...
const VeloraDashboard = () => {
  ...
  return (
    <div className="w-full h-screen bg-black text-white font-mono">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
        <div>
          ...
          <div className="text-xs mt-1 flex items-center gap-2">
            {isConnected ? (
              <><Wifi size={12} className="text-green-400" /> <span className="text-green-400">WebSocket Connected</span></>
            ) : (
              <><WifiOff size={12} className="text-red-400" /> <span className="text-red-400">Connecting...</span></>
            )}
          </div>
        </div>
      ...
      <div className="flex h-[calc(100vh-100px)] overflow-auto">
        <div className="flex-1 p-4 min-w-[1200px]">
          <div className="bg-gray-800 rounded-lg p-4 relative">
            <div className="absolute top-6 right-6 bg-black/80 px-6 py-3 rounded border border-gray-600 z-10">
              <div className="text-lg font-bold">LAP {Math.max(...cars.map(c => c.lap), 0)}/{totalLaps}</div>
            </div>
            <svg ref={svgRef} width="1100" height="800" className="mx-auto" />
          </div>
          
          <div className="mt-4 bg-gray-800 rounded-lg p-4">
            <div className="text-sm font-bold mb-2 flex items-center gap-2">
              <Activity size={16} />
              CAR SPEEDS OVER TIME (D3.js Real-time Chart)
            </div>
            <svg ref={chartRef} width="1100" height="120" />
          </div>
        </div>
